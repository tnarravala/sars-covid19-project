...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb  8 23:30:48 2021
...
def compare(inp,co):
    ...
    for t_rate in t_rates:
        city, school, commuter = simulate(testing = True, vaccinating = True, ct_rate = city_test_rate,
                                          ...
                                          fix_s_beta = fix_school_beta,day_frac = daytime_fraction,County = co)
        cities.append(city)
        schools.append(school)
        commuters.append(commuter)
    ...
    for i in range(len(t_rates)):
        ...
        GH_combined = [schools[i].GH[t] + commuters[i].GH[t] for t in range(len(schools[i].GH))]
...
app.layout = html.Div(
    ...
    html.Div([ dcc.Input(id="D/N",type='number',placeholder="D/N...",
            min=0, max=1, step=0.0001,style = {'width':'10%','display':'inline-block'},className='six columns',value = 1 / 2
    ...
    #html.Div(html.H4(id='output-container',style={'display':'inline-block','margin-right':'20%'})),
         html.Div([dcc.Graph(id="fig",figure = school_testing_cost1(0.0015,0.02,25,1 / 2,'IL-Cook',5500,8500,11600),
            ...
            dcc.Graph(id='fig2',figure = compare('IH','IL-Cook'),style={'display':'inline-block'},)
                  ]),
