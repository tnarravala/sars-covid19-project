...
indiavac = 'india/indiavaccine.csv'
...
class State:
	def __init__(self, state, paras_df, pop_df, confirm_df, death_df,vaccine_df):
		...
		#new = vaccine_df[vaccine_df['state'] == state]
...
def improvements():
	table = []
	col = ['state', 'G improvement', 'D improvement']
	for state in states:
		G1 = G1s[state]
		D1 = D1s[state]
		G2 = G2s[state]
		...
		table.append([state_dict[state], G_improvement, D_improvement])
	...
	table.append((['India', G_improvement, D_improvement]))
